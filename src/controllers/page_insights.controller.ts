...
import pageInsightsService from "../services/page_insights.service";
...
export class PageInsightsController extends BaseController {
  getPageInsights = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
    try {
      ...
      const formattedInsights = ResponseFormatter.formatPageInsights(pageId, insights as never);
  ...
  getPageMetrics = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
    try {
      const { pageId, metricName, since, until } = req.params as Record<string, string>;
      ...
      const formattedMetrics = ResponseFormatter.formatPageInsights(pageId, metrics as never);
  ...
  createPageInsight = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
    try {
      ...
      const formattedInsight = ResponseFormatter.formatPageInsights(insightData.page_id as string, [insight as never]);
